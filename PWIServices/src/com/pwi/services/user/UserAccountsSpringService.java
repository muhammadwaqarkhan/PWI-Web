...
public class UserAccountsSpringService extends SpringServiceBase {
	...
	public IResponseHandler varifedUser(UserAccountsInDTO o) {
		...
		UserAccounts product = jdbcTemplate.queryForObject("SELECT * FROM useraccounts WHERE userid = ? ",
				new Object[] { new Integer(1) }, new RowMapper<UserAccounts>() {
					@Override
					public UserAccounts mapRow(ResultSet rs, int row) throws SQLException {
	...
	public IResponseHandler execute(IRequestHandler o) {
		...
		if (o instanceof UserAccountsInDTO) {
			...
			UserAccounts product = jdbcTemplate.queryForObject("SELECT * FROM PRODUCTS WHERE ID = ? ",
					new Object[] { new Integer(1) }, new RowMapper<UserAccounts>() {
						@Override
						public UserAccounts mapRow(ResultSet rs, int row) throws SQLException {
							UserAccounts user = new UserAccounts();

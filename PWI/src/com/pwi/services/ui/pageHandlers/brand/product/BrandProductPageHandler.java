...
public class BrandProductPageHandler extends BasePageHandler implements IPageHandler {
	...
	protected IResponseHandler getBrandProducts(HttpServletRequest request) {
		...
		IResponseHandler response = getServiceExecutor().callService(getService(), "FetchBrandProduct",
				new StoreProductDTO());
	...
	protected IResponseHandler deleteStoreProduct(HttpServletRequest request) {
		String brandProductID = request.getParameter("selectedID");
		brandProductID = brandProductID == null ? "-1" : brandProductID;
		BrandProductDTO dto = new BrandProductDTO();
		dto.setBrandProductID(Long.valueOf(brandProductID));

		IResponseHandler response = getServiceExecutor().callService(getService(), "DeleteBrandProduct", dto);

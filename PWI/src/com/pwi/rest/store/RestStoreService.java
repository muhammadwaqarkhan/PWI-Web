...
public class RestStoreService {
	...
	@Produces(MediaType.TEXT_XML)
	@Path("/addStore")
	public String addStore(@QueryParam("branchID") String branchID, @QueryParam("storeName") String storeName,
			@QueryParam("city") String city, @QueryParam("country") String country,
			@QueryParam("postalCode") String postalCode, @QueryParam("street") String street) {
	...
	@Produces(MediaType.TEXT_XML)
	@Path("/updateStore")
	public String updateStore(@QueryParam("branchID") String branchID, @QueryParam("storeName") String storeName,
			@QueryParam("city") String city, @QueryParam("country") String country,
			@QueryParam("postalCode") String postalCode, @QueryParam("street") String street) {
	...
	public String updateStoreItemQuantity(@QueryParam("storeID") String storeID,
			@QueryParam("productID") String productID, @QueryParam("quantity") String quantity) {
	...
	/*
	 * public List<JaxBranchOutDTO> getBranchStoreQuantity(
	 * @QueryParam("branchID") String branchID,
	 * 
	 * @QueryParam("branchName") String branchName) { branchID = branchID ==
	 * null ? "0" : branchID; branchName = branchName == null ? "0" :
	 * branchName;
	 * 
	 * BranchDTO dto = new BranchDTO(); dto.setBranchName(branchName);
	 * dto.setBranchID(Long.valueOf(branchID));
	 * 
	 * 
	 * List<JaxBranchOutDTO> outDTO = new ArrayList<JaxBranchOutDTO>();
	 * ServiceExecutor executor= BasePageHandler.getServiceExecutor();
	 * ServiceBase base = new BranchService(); Object object =
	 * executor.callService(base, "webServiceBranch", dto);
	 * 
	 * 
	 * if(object instanceof BranchStoreProductOutDTO ) {
	 * for(StoreProductQuantitySize branch :
	 * ((BranchStoreProductOutDTO)object).getQuantitySize()) { JaxBranchOutDTO
	 * dto = new JaxBranchOutDTO(); dto.setBranchName(branch.getBranchName());
	 * dto.setStoreName(branch.getStoreName());
	 * dto.setQuantity(branch.getQuantity()); dto.setSize(branch.getSize());
	 * 
	 * outDTO.add(dto); } }
	 * 
	 * return outDTO; }
